...
function renderSubjectDetail() {
  ...
  $("#detail-files").innerHTML = files.map(file => isVideo(file) ? `<div class="detail-file"><b>${esc(file.display_name || file.original_name)}</b><small>${esc(file.topic_name || "Subject material")}</small><video controls preload="metadata" src="/uploads/${encodeURIComponent(file.stored_name)}"></video></div>` : `<div class="detail-file"><b>${esc(file.display_name || file.original_name)}</b><small>${esc(file.topic_name || "Subject material")}</small><a class="secondary file-link" href="/uploads/${encodeURIComponent(file.stored_name)}" target="_blank" rel="noopener">Open file</a></div>`).join("") || `<p class="empty-state">No files are attached to this subject yet. Upload them from Files and choose this subject.</p>`;
}
...
async function loadGroups() { state.groups = await api("/api/groups"); $("#group-grid").innerHTML = state.groups.map(g => `<article class="group-card"><h3>${esc(g.name)}</h3><p>${esc(g.description || "A shared space for focused preparation.")}</p><span class="members">${g.member_count} member${g.member_count === 1 ? "" : "s"} · by ${esc(g.owner_name)}</span>${g.joined ? `<button class="secondary" data-open-group="${g.id}">Open room →</button>${g.owner_id === state.user.id ? `<small class="muted">Invite code: ${esc(g.invite_code)}</small>` : ""}` : `<button class="primary" data-join-group="${g.id}">Join group</button>`}</article>`).join("") || `<div class="empty-state">No groups yet. Create one and invite your study partners.</div>`; }
